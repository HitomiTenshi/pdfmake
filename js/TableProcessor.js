...
class TableProcessor {
  ...
  getCellContentHeight(cell, items) {
    ...
    !contentHeight && (contentHeight = items.reduce((p, v) => {
      var _v$item$inlines$, _ref, _v$item$__nodeRef$lin, _v$item$__nodeRef, _ref2, _item$height;
      const item = v.item.inlines ? (_v$item$inlines$ = v.item.inlines[0]) != null ? _v$item$inlines$ : null : v.item;
      const lineHeight = (_ref = (_v$item$__nodeRef$lin = (_v$item$__nodeRef = v.item.__nodeRef) == null ? void 0 : _v$item$__nodeRef.lineHeight) != null ? _v$item$__nodeRef$lin : v.item._height) != null ? _ref : v.item.h;
      let height = (_ref2 = (_item$height = item.height) != null ? _item$height : item.h) != null ? _ref2 : 0;
          ...
          if (cellHeight) {
            ...
            items.forEach(x => {
              ...
              if (x && x.item) {
                ...
                x.item.type && (0, _tools.offsetVector)(x.item, 0, Math.max(0, offsetTop) - paddingTop);
